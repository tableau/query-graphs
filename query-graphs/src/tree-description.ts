...
export type TreeNode = {
    ...
    costlyScan?: boolean;
    // Marks a high-volume scan: one whose processed-rows clear the high-volume threshold, independent of
    // selectivity. Used to tint the `processed-rows` property row (indigo) to match the node highlight.
    // Recomputed at render time for adjustable plans.
    ...
    cpuTime?: number;
    // Measured peak memory (bytes) for this operator (memory-hotspot input).
    memoryBytes?: number;
...
export interface TreeDescription {
    ...
    /// Total peak memory (bytes) across all operators, used to compute each operator's memory share
    /// (and thus the memory-hotspot tint) at render time. Only set when `adjustableHighlights` is true.
