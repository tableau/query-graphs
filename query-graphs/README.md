...
* **Format loaders** — `hyper.ts`, `umbra.ts`, `duckdb.ts`, `mariadb.ts`, `postgres.ts`, `tableau.ts`, and the generic `json.ts`/`xml.ts` fallbacks (`src/loaders/`), each turning plan text into a `TreeDescription`.
