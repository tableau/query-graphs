...
test("following a source document requests its linked nodes", () => {
    ...
    assert.equal(store.getState().nodeIdsToReveal, undefined);

    store.getState().setFollowSourceDocument("query", true);
    assert.deepEqual(store.getState().nodeIdsToReveal, new Set(["0", "1"]));
...
// A newer tree interaction cancels source-driven navigation before its hover-intent delay can move the viewport.
test("tree interactions cancel pending source navigation", () => {
    const sourceNode: TreeNode = {sourceLocations: [{documentId: "query", from: 0, to: 5}]};
    const hoveredNode: TreeNode = {};
    const store = graphStore([sourceNode, hoveredNode]);

    store.getState().setFollowSourceDocument("query", true);
    store.getState().setActiveSourceLocations("query", [{documentId: "query", from: 0, to: 5}]);
    assert.deepEqual(store.getState().nodeIdsToReveal, new Set(["0"]));
    ...
    store.getState().setHoveredNodeId(undefined);
    store.getState().setActiveSourceLocations("query", [{documentId: "query", from: 0, to: 5}]);
    assert.ok(store.getState().nodeIdsToReveal);
    store.getState().toggleExpandedSubtree("0");
    assert.equal(store.getState().nodeIdsToReveal, undefined);
});
...
test("source navigation projects hidden nodes to their visible ancestors", () => {
    ...
    store.getState().setActiveSourceLocations("query", []);
    store.getState().setActiveSourceLocations("query", [{documentId: "query", from: 0, to: 5}]);
    assert.deepEqual(store.getState().nodeIdsToReveal, new Set(["1"]));
});
