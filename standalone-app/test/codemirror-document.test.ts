...
test("the document editor schedules highlight navigation only while following is enabled", async () => {
    const fixture = await createFixture();
    const originalRevealHighlightedRange = rangeLinking.revealHighlightedRange;
    let revealRequests = 0;
    rangeLinking.revealHighlightedRange = () => revealRequests++;
    try {
        ...
        await fixture.render({highlightedRanges, followHighlightedRanges: true});
        await React.act(async () => new Promise((resolve) => setTimeout(resolve, 175)));
        ...
        await fixture.render({
            ...
            followHighlightedRanges: true,
        });
        await fixture.render({highlightedRanges, followHighlightedRanges: false});
        await React.act(async () => new Promise((resolve) => setTimeout(resolve, 175)));
        assert.equal(revealRequests, 1);
    } finally {
        rangeLinking.revealHighlightedRange = originalRevealHighlightedRange;
        await fixture.cleanup();
    }
});
