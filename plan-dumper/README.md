...
* `dump-plans.py` — runs each query against Hyper (and optionally Postgres and DuckDB) and writes the `EXPLAIN` output as JSON into `standalone-app/examples/<db>/`.
...
* `*-steps.sql` → `EXPLAIN (FORMAT JSON, OPTIMIZE STEPS)` — the optimizer's intermediate steps (Hyper only; skipped for Postgres and DuckDB).
...
To also dump DuckDB plans, `pip3 install duckdb`; no server or extra setup is needed. If `duckdb` is not installed, the DuckDB section is skipped automatically.
